    ...
    <BentoCard className="p-8 md:p-10 flex flex-col justify-between min-h-[300px] md:min-h-[350px]" delay={0.1}>
      <div className="space-y-4 md:space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 text-blue-400 font-bold uppercase tracking-widest text-[9px] md:text-[10px] bg-blue-500/20 px-3 md:px-4 py-1.5 rounded-full w-fit border border-blue-400/20">
        ...
        <div className="space-y-1">
          <h2 className="text-3xl md:text-4xl font-black text-white">{data.city}</h2>
          <p className="text-sm md:text-base text-white/60 font-medium capitalize">{currentTime} • {data.description}</p>
        </div>
      </div>
      <div>
        <div className="flex items-baseline">
          <AnimatedTemp value={convert(data.temp)} unit={isCelsius ? '°C' : '°F'} className="text-7xl md:text-8xl font-black tracking-tighter text-white" />
          <span className="text-2xl md:text-4xl font-light text-white/30 ml-2">{isCelsius ? '°C' : '°F'}</span>
        </div>
        <div className="flex items-center space-x-4 mt-4 text-white/60 font-bold text-sm md:text-base">
